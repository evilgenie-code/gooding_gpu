...
void sigmaxGpu(double y, double sig = 0.0, double dsigdx = 0.0, double d2sigdx2 = 0.0, double d3sigdx3 = 0.0)
{
    double powers[25];
    int i;
    // preload the factors[an]
...
__global__ void LancasterBlanchardGpu(double x, double q, double m, double* Tprod)
{
    ...
    if (tid == 0)
    {
        if (x == 1) // exactly parabolic; solutions known exactly
        {
            ...
            // evaluate sigma
        {
            sigmaxGpu(-E, sig1, dsigdx1, d2sigdx21, d3sigdx31);
            sigmaxGpu(-E * q * q, sig2, dsigdx2, d2sigdx22, d3sigdx32);
            Tprod[0] = sig1 - pow(q, 3) * sig2;
        }
        else // all other cases
        {
    ...
    else if (tid == 1)
    {
        ...
        else // all other cases
        {
            ...
            g = x * z - q * E;
            d = (E < 0) * (atan2(f, g) + M_PI * m) + (E > 0) * logf(MAX(0, f + g));
            Tprod[1] = (4 - 4 * pow(q, 3) * x / z - 3 * x * (2 * (x - q * z - d / y) / E)) / E;
        }
    }
    else if(tid == 2)
    {
        if (x == 1) // exactly parabolic; solutions known exactly
        {
            Tprod[2] = (4 / 5 * (pow(q, 5) - 1)) + 120 / 70 * (1 - pow(q, 7));
        }
        else if (fabs(x - 1) < 1e-2) // near - parabolic; compute with series
            // evaluate sigma
        {
            sigmaxGpu(-E, sig1, dsigdx1, d2sigdx21, d3sigdx31);
            sigmaxGpu(-E * q * q, sig2, dsigdx2, d2sigdx22, d3sigdx32);;
            Tprod[2] = (2 * x * (pow(q, 5) * dsigdx2 - dsigdx1)) / x + 4 * pow(x, 2) * (d2sigdx21 - pow(q, 7) * d2sigdx22);
        }
        else // all other cases
        {
            // compute all substitution functions
            ...
            d = (E < 0) * (atan2(f, g) + M_PI * m) + (E > 0) * logf(MAX(0, f + g));
            Tprod[2] =
                    (-4 * pow(q, 3) / z * (1 - pow(q, 2) * pow(x, 2) / pow(z, 2)) - 3 * (2 * (x - q * z - d / y) / E) -
                     3 * x * ((4 - 4 * pow(q, 3) * x / z - 3 * x * (2 * (x - q * z - d / y) / E)) / E)) / E;
        }
    }
    else if(tid == 3)
    {
        if (x == 1) // exactly parabolic; solutions known exactly
        {
            Tprod[0] = 4.0 / 3 * (1 - pow(q, 3));
            double Tprod1 = 4.0 / 5 * (pow(q, 5) - 1);
            double Tprod2 = Tprod1 + 120.0 / 70 * (1 - pow(q, 7));
            Tprod[3] = 3 * (Tprod2 - Tprod1) + 2400.0 / 1080 * (pow(q, 9) - 1);
        }
        else if (fabs(x - 1) < 1e-2) // near - parabolic; compute with series
            // evaluate sigma
        {
            ...
            double Tprod1 = 2 * x * (pow(q, 5) * dsigdx2 - dsigdx1);
            double Tprod2 = Tprod1 / x + 4 * pow(x, 2) * (d2sigdx21 - pow(q, 7) * d2sigdx22);
            Tprod[3] = 3 * (Tprod2 - Tprod1 / x) / x + 8 * x * x * (pow(q, 9) * d3sigdx32 - d3sigdx31);
        }
        else // all other cases
        {
            // compute all substitution functions
            ...
            g = x * z - q * E;
            d = (E < 0) * (atan2(f, g) + M_PI * m) + (E > 0) * logf(MAX(0, f + g));
            double Tprod0 = 2 * (x - q * z - d / y) / E;
            double Tprod1 = (4 - 4 * pow(q, 3) * x / z - 3 * x * Tprod0) / E;
            double Tprod2 = (-4 * pow(q, 3) / z * (1 - pow(q, 2) * pow(x, 2) / pow(z, 2)) - 3 * Tprod0 - 3 * x * Tprod1) / E;
            Tprod[3] = (4 * pow(q, 3) / pow(z, 2) * ((1 - pow(q, 2) * pow(x, 2) / pow(z, 2)) + 2 * pow(q, 2) * x / pow(z, 2) * (z - x)) - 8 * Tprod1 - 7 * x * Tprod2) / E;

        }
    }
}

//double getXM(double xMpi, double phr)
//{
//    if (phr < M_PI)
//    {
...
__global__ void getXMHalleysMethodGpu(float xM, float q, float revs)
{
    int iterations;
    ...
    while (fabs(Tp) > tol)
    {
        ...
        LancasterBlanchardGpu<<<1,4>>>(xM, q, revs, tProd);
...
{
    getXMHalleysMethodGpu <<<1,1>>>(xM, q, revs);
}
...
void lamdert(const float* r1, const float* r2, float t, int revs, int lw, double mu)
{
    ...
    double  r1Unit[3], r2Unit[3], crsProd[3], ucrsProd[3], th1Unit[3], th2Unit[3],
            r1Double[3], r2Double[3], Tprod0[4] = { T0, 0.0, 0.0, 0.0 }, Td, phr, v1[3], v2[3], Tprod[4];
